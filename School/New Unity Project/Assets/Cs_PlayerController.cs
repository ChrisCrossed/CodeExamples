...
public class Cs_PlayerController : MonoBehaviour
{
    ...
    public void SetCameraPosition(GameObject go_CameraPos_ = null)
    {
        ...
        */
        ...
        else // Going away from player
        {
            if(cameraState == Enum_CameraState.Lerp_ToPlayer)
            {
                go_Camera_TempPos = go_CameraPos_;
                cameraState = Enum_CameraState.Lerp_FromPlayer;
            ...
            else if(cameraState == Enum_CameraState.OnPlayer)
            {
                go_Camera_TempPos = go_CameraPos_;
                cameraState = Enum_CameraState.Lerp_FromPlayer;
        ...
        */
        ...
        if (go_CameraPos_ == null)
        {
            if (cameraState == Enum_CameraState.Lerp_FromPlayer)
            {
                // Create a lerp-able object in regards to where the camera currently is
                GameObject tempObj = new GameObject();
                tempObj.transform.position = go_Camera.transform.position;
                tempObj.transform.rotation = go_Camera.transform.rotation;
                go_Camera_TempPos = tempObj;

                // Set the proper lerp-able time based on how far it has moved
                float f_FullDistance = Vector3.Distance(go_Camera_TempPos.transform.position, go_Camera_DefaultPos.transform.position);
                float f_MidDistance = Vector3.Distance(tempObj.transform.position, go_Camera_TempPos.transform.position);
                float f_Distance = f_MidDistance / f_FullDistance;

                // Set camera lerp timer
                cameraLerpTime_Curr = f_Distance * cameraLerpTime;

                // Set camera lerp state
                cameraState = Enum_CameraState.Lerp_ToPlayer;
            }
            else if (cameraState == Enum_CameraState.OnTempPoint)
            {
        ...
        else // Going away from player
        {
            if (cameraState == Enum_CameraState.Lerp_ToPlayer)
            {
                ...
                // Set the proper lerp-able time based on how far it has moved
                float f_FullDistance = Vector3.Distance(go_Camera_DefaultPos.transform.position, go_Camera_TempPos.transform.position);
                float f_MidDistance = Vector3.Distance(tempObj.transform.position, go_Camera_TempPos.transform.position);
                float f_Distance = f_MidDistance / f_FullDistance;
        ...
        /*
        if(cameraState == Enum_CameraState.OnPlayer || cameraState == Enum_CameraState.OnTempPoint)
        {
            if (go_CameraPos_ == null)
            {
                cameraState = Enum_CameraState.Lerp_ToPlayer;
        ...
        // The camera is currently moving TO the player, and now needs to go FROM the player
        else if(cameraState == Enum_CameraState.Lerp_ToPlayer)
        {
            // Create a lerp-able object in regards to where the camera currently is
            GameObject tempObj = new GameObject();
            tempObj.transform.position = go_Camera.transform.position;
        ...
        else if(cameraState == Enum_CameraState.Lerp_FromPlayer)
        {
            // Create a lerp-able object in regards to where the camera currently is
            GameObject tempObj = new GameObject();
            tempObj.transform.position = go_Camera.transform.position;
